...
class DateDeserializer extends JsonDeserializer<Object> {
    ...
    public Object deserialize(final JsonParser p, final DeserializationContext ctxt, final Object target) throws IOException {
        ...
        if (value != null) {
            ...
            if (stringValue.endsWith(DateSerializer.DATE_SUFFIX)) {
                result = parseDate(stringValue.substring(0, stringValue.length() - 6));
            } else {
                result = value;
            }
        } else {
    ...
    private Date parseDate(final String dateString) {
        ...
        try {
            ...
            try {
                date = DateSerializer.simpleDateFormatTimeStamp.parse(dateString);
            } catch (final ParseException e) {
                log.info("Could not parse date: " + dateString);
                try {
                    date = DateSerializer.simpleDateFormat.parse(dateString);
...
class DateSerializer extends JsonSerializer<Object> {
    ...
    private static final String DATE_FORMAT_WITH_TIMESTAMP = "yyyy/MM/dd HH:mm:ss:SSS";
    static final SimpleDateFormat simpleDateFormatTimeStamp = new SimpleDateFormat(DATE_FORMAT_WITH_TIMESTAMP);
    static final SimpleDateFormat simpleDateFormat = new SimpleDateFormat(DATE_FORMAT);
    ...
    public void serialize(final Object o, final JsonGenerator jsonGenerator, final SerializerProvider serializerProvider) throws IOException, JsonProcessingException {
        if (o instanceof Date) {
            ...
            final String dateValue = simpleDateFormatTimeStamp.format(date);
