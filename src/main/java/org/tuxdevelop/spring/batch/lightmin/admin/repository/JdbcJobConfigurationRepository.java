...
public class JdbcJobConfigurationRepository implements JobConfigurationRepository, InitializingBean {
    ...
    private static class JobConfigurationDAO {
        ...
        private static final String UPDATE_STATEMENT =
                "UPDATE " + TABLE_NAME + " SET " + JobConfigurationDomain.JOB_NAME + "" +
                        " = ? , " + JobConfigurationDomain.JOB_INCREMENTER + " = ? WHERE " + JobConfigurationDomain
                        .JOB_CONFIGURATION_ID + " = ?";

        private static final String DELETE_STATEMENT = "DELETE FROM " + TABLE_NAME + " WHERE " + JobConfigurationDomain
                .JOB_CONFIGURATION_ID + " = ?";

        private static final String GET_JOB_CONFIGURATION_ID_COUNT_STATEMENT =
        ...
        public JobConfigurationDAO(final JdbcTemplate jdbcTemplate, final String tablePrefix) {
            ...
            this.simpleJdbcInsert =
                    new SimpleJdbcInsert(jdbcTemplate).withTableName(String.format(TABLE_NAME, tablePrefix))
                            .usingGeneratedKeyColumns(JobConfigurationDomain.JOB_CONFIGURATION_ID);
        }
        ...
        public void update(final JobConfiguration jobConfiguration) {
            ...
            jdbcTemplate.update(sql, new Object[]{jobConfiguration.getJobName(),
                    jobConfiguration.getJobIncrementer().getIncrementerIdentifier(), jobConfiguration
                    .getJobConfigurationId()}, new int[]{Types.VARCHAR, Types.VARCHAR, Types.NUMERIC});
        }
        ...
        public Long getJobConfigurationIdCount(final Long jobConfiguration) {
            ...
            return jdbcTemplate
                    .queryForObject(sql, new Object[]{jobConfiguration}, new int[]{Types.NUMERIC}, Long.class);
        }
        ...
        private Map<String, Object> map(final JobConfiguration jobConfiguration) {
            ...
            keyValues.put(JobConfigurationDomain.JOB_INCREMENTER,
    ...
    private static class JobSchedulerConfigurationDAO {
        ...
        private static final String UPDATE_STATEMENT =
                "UPDATE " + TABLE_NAME + " SET " + JobSchedulerConfigurationDomain
                        .CRON_EXPRESSION + " = ? , " + JobSchedulerConfigurationDomain.FIXED_DELAY + " = ? , " +
                        JobSchedulerConfigurationDomain.INITIAL_DELAY + " = ? , " + JobSchedulerConfigurationDomain
                        .SCHEDULER_TYPE + " = ?, " + JobSchedulerConfigurationDomain.TASK_EXECUTOR_TYPE +
                        " = ? WHERE " + JobSchedulerConfigurationDomain.JOB_CONFIGURATION_ID + " = ? ";
        ...
        public JobSchedulerConfigurationDAO(final JdbcTemplate jdbcTemplate, final String tablePrefix) {
            this.jdbcTemplate = jdbcTemplate;
            this.tablePrefix = tablePrefix;
            this.simpleJdbcInsert = new SimpleJdbcInsert(jdbcTemplate).withTableName(String.format(TABLE_NAME,
                    tablePrefix)).usingGeneratedKeyColumns(JobSchedulerConfigurationDomain.ID);
        }
        ...
        public void attacheJobSchedulerConfiguration(final JobConfiguration jobConfiguration) {
            ...
            final JobSchedulerConfiguration jobSchedulerConfiguration = jdbcTemplate.queryForObject
                    (sql, new JobSchedulerConfigurationRowMapper(), jobConfiguration
        ...
        public void update(final JobConfiguration jobConfiguration) {
            ...
            final String sql = String.format(UPDATE_STATEMENT, tablePrefix);
            final Object[] parameters = {jobSchedulerConfiguration.getCronExpression(), jobSchedulerConfiguration
                    .getFixedDelay(), jobSchedulerConfiguration.getInitialDelay(), jobSchedulerConfiguration
                    .getJobSchedulerType().getId(), jobSchedulerConfiguration.getTaskExecutorType().getId(),
        ...
        private Map<String, Object> map(final JobConfiguration jobConfiguration) {
            ...
            keyValues.put(JobSchedulerConfigurationDomain.JOB_CONFIGURATION_ID,
                    jobConfiguration.getJobConfigurationId());
            keyValues.put(JobSchedulerConfigurationDomain.CRON_EXPRESSION, jobSchedulerConfiguration.getCronExpression
                    ());
    ...
    private static class JobConfigurationParameterDAO {
        ...
        public JobConfigurationParameterDAO(final JdbcTemplate jdbcTemplate, final String tablePrefix) {
            ...
            this.tablePrefix = tablePrefix;
            this.simpleJdbcInsert = new SimpleJdbcInsert(jdbcTemplate).withTableName(String.format(TABLE_NAME,
                    tablePrefix)).usingGeneratedKeyColumns(JobConfigurationParameterDomain.ID);
            this.dateFormat = DateFormat.getDateInstance(SimpleDateFormat.FULL);
        }
        ...
        public void attacheParameters(final JobConfiguration jobConfiguration) {
            ...
            final List<JobConfigurationParameter> jobConfigurationParameters = jdbcTemplate.query(sql, new
                    JobConfigurationParameterRowMapper(), jobConfigurationId);
        ...
        private Object createValue(final String value, final ParameterType parameterType) {
            if (ParameterType.LONG.equals(parameterType)) {
                ...
                throw new SpringBatchLightminApplicationException("Unsupported ParameterType: " + parameterType
    ...
    private static class JobSchedulerConfigurationRowMapper implements RowMapper<JobSchedulerConfiguration> {
        ...
        public JobSchedulerConfiguration mapRow(final ResultSet resultSet, final int rowNum) throws SQLException {
            ...
            jobSchedulerConfiguration
                    .setCronExpression(resultSet.getString(JobSchedulerConfigurationDomain.CRON_EXPRESSION));
            ...
            final JobSchedulerType jobSchedulerType = JobSchedulerType.getById(resultSet.getLong
                    (JobSchedulerConfigurationDomain.SCHEDULER_TYPE));
            jobSchedulerConfiguration.setJobSchedulerType(jobSchedulerType);
            final TaskExecutorType taskExecutorType = TaskExecutorType.getById(resultSet.getLong
    ...
    private static class JobConfigurationRowMapper implements RowMapper<JobConfiguration> {
        ...
        public JobConfiguration mapRow(final ResultSet resultSet, final int rowNum) throws SQLException {
            ...
            final JobIncrementer jobIncrementer = JobIncrementer.getByIdentifier(resultSet.getString
                    (JobConfigurationDomain.JOB_INCREMENTER));
    ...
    private static class JobConfigurationParameterRowMapper implements RowMapper<JobConfigurationParameter> {
        ...
        @Override
        public JobConfigurationParameter mapRow(final ResultSet resultSet, final int rowNum)
                throws SQLException {
            final JobConfigurationParameter jobConfigurationParameter = new JobConfigurationParameter();
            jobConfigurationParameter
                    .setParameterName(resultSet.getString(JobConfigurationParameterDomain.PARAMETER_NAME));
            jobConfigurationParameter
                    .setParameterValue(resultSet.getString(JobConfigurationParameterDomain.PARAMETER_VALUE));
            jobConfigurationParameter
                    .setParameterType(resultSet.getLong(JobConfigurationParameterDomain.PARAMETER_TYPE));
            return jobConfigurationParameter;
        }
    }
